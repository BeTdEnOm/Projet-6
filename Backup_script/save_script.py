...
print("Sauvegarde de la base de données effectuée ("+ DB_NAME + ".sql).")
print("")
...
print("Ouverture et création de l'archive...")
with tarfile.open(BACKUP_NAME + '.tar.gz', mode='w:gz') as BACKUP :
  BACKUP.add(FILE1, recursive = True)
  print("Ajout du dossier" + str(FILE1) + " dans l'archive...")
  BACKUP.add(FILE2, recursive = True)
  print("Ajout du dossier" + str(FILE2) + " dans l'archive...")
  DB_BACKUP = DB_NAME + ".sql"
  BACKUP.add(DB_BACKUP)
  print("Ajout du fichier de sauvegarde de la BDD " + str(DB_BACKUP) + " dans l'archive...")
print("Fermeture du fichier d'archive compressé " +BACKUP_NAME + '.tar.gz' " .")
print("")
print("La sauvegarde de " + BACKUP_NAME + ".tar.gz" + " à été créée et a été placé \ndans le dossier /backup.")
os.remove(DB_BACKUP)
print("")
